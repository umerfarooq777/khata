  ...
  if (documents) {
    ...
    for (let j = 0; j < LedgerEntries.length; j++) {
      ...
      if(debit>=credit){
          ...
          IncomeStatement.push({title:el[0],CD:"Debit",generalAcc:genac,amount:debit-credit})
      }
      else if(credit>debit){
          
          IncomeStatement.push({title:el[0],CD:"Credit",generalAcc:genac,amount:credit-debit})
      ...
      <Table  aria-label="customized table">
        <TableHead>
          ...
          IncomeStatement.map((row,index) => (
            <TableBody>
            <StyledTableRow key={index}>
              {(row.generalAcc==="Liabilities" ||  row.generalAcc==="Capital") && <>
              <StyledTableCell align="left">{row.title}</StyledTableCell>
